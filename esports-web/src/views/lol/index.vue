<template>
  <div class="lol-home-page">
    <div class="hero-section">
      <div class="hero-content">
        <h1>英雄联盟板块</h1>
        <p>欢迎来到英雄联盟电竞专区，这里是高校英雄联盟战队的集结地</p>
        <div class="hero-buttons">
          <el-button type="primary" size="large" @click="$router.push('/lol/team')">浏览战队</el-button>
          <el-button size="large" @click="$router.push('/lol/match')">查看约战</el-button>
        </div>
      </div>
    </div>
    ...
    <div class="section-wrapper">
      <div class="section-header">
        ...
        <el-button link @click="$router.push('/lol/team')">全部战队 <el-icon><ArrowRight /></el-icon></el-button>
      </div>
      <el-row :gutter="20" v-loading="loading">
        <el-col :span="6" v-for="team in hotTeams" :key="team.id">
          <el-card class="team-item-card" :body-style="{ padding: '0px' }" shadow="hover" @click="$router.push(`/lol/team/${team.id}`)">
            ...
            <div class="team-info">
              ...
              <div class="team-footer">
                ...
                <el-button type="primary" link @click.stop="$router.push(`/lol/team/${team.id}`)">详情</el-button>
              </div>
            </div>
          </el-card>
        </el-col>
    ...
    <div class="section-wrapper" style="margin-top: 40px;">
      <div class="section-header">
        ...
        <el-button link @click="$router.push('/lol/match')">全部约战 <el-icon><ArrowRight /></el-icon></el-button>
      </div>
      <el-row :gutter="20" v-loading="loading">
        <el-col :span="6" v-for="room in latestMatches" :key="room.id">
          <el-card class="match-room-card" shadow="hover" @click="$router.push(`/lol/match/${room.id}`)">
            ...
            <div class="room-footer">
              <el-button 
                ...
                type="primary" 
                class="join-btn" 
                @click.stop="$router.push(`/lol/match/${room.id}`)"
              >立即应战</el-button>
              <el-button 
...
const fetchData = async () => {
  ...
  try {
    // 获取热门战队
    const teamsRes = await request.get('/team/list', { params: { gameProject: 'LOL', page: 1, pageSize: 4 } });
    hotTeams.value = teamsRes.data || [];

    // 获取最新约战
    const matchesRes = await request.get('/match-room/list', { params: { gameProject: 'LOL', page: 1, pageSize: 4 } });
    latestMatches.value = matchesRes.data || [];
  } catch (err) {
    console.error('获取LOL板块数据失败', err);
  } finally {
    loading.value = false;
  }
};
